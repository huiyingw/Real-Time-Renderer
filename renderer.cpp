...
void Renderer::Render()
{
    ...
    {
        ImGui::Checkbox("Show shadow map", &mShadowDepthVis);
    ...
    if (*mShadowSP)
    {
        ...
        GLint SCENE_MODELVIEWPROJECTION_UNIFORM_LOCATION = glGetUniformLocation(*mShadowSP, "lightModelProjection");
        ...
        for (uint32_t instanceID : mScene->Instances)
        {
            ...
            glm::mat4 lightModelProjection = lightWorldProjection * modelWorld;

            glProgramUniformMatrix4fv(*mShadowSP, SCENE_MODELVIEWPROJECTION_UNIFORM_LOCATION, 1, GL_FALSE, value_ptr(lightModelProjection));
    ...
    if (*mSceneSP)
    {
        ...
        for (uint32_t instanceID : mScene->Instances)
        {
            ...
            for (const Transform* curr_transform = transform;
                 ...
                 curr_transform = &mScene->Transforms[curr_transform->ParentID])
            {
                normal_ModelWorld = mat3_cast(curr_transform->Rotation) * normal_ModelWorld;
                normal_ModelWorld = glm::mat3(scale(1.0f / curr_transform->Scale)) * normal_ModelWorld;
    ...
    if (mShadowDepthVis && *mDepthVisSP)
    {
        ...
        glm::mat4 transform2D = glm::translate(glm::vec3((float)(mBackbufferWidth - depthBufferVisSize), (float)(mBackbufferHeight - depthBufferVisSize), 0.0f)) * glm::scale(glm::vec3(depthBufferVisSize, depthBufferVisSize, 0.0f));
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_REF_TO_TEXTURE); glDisable(GL_BLEND);
